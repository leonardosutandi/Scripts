...
% std_plottf( ersptimes, erspfreqs, erspdata, 'titles', title,'caxis', [-1 1])

% Spatial against Neutral (SaN) formula
erspdataSaN = {(erspdata_NG_Left{1,1} - erspdata_NG_Neutral{1,1})};

if length(size(erspdataSaN)) >= 4
    erspdataSaN = mean(erspdataSaN,4);
else
    erspdataSaN = erspdataSaN;
end
% Evoked (total power avg - induce power avg)

% Replot
% std_plottf(ersptimes, erspfreqs, erspdataSaN, 'titles', title,'caxis', [-1 1]);
tftopo(erspdataSaN, ersptimes, erspfreqs);
