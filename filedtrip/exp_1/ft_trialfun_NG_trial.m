function [trl, event] = ft_trialfun_NG_trial(cfg)

% FT_TRIALFUN_NG_TRIAL is a trial function searching for events
% of type "STATUS" and specifically for a trigger with value 1 (no gratings condition),
...
for j = 1:(length(value)-2)
  ...
  trig2 = value(j+1);
  trig3 = value(j+2);
  if trig1 == trig_trialType(1) && ismember(trig2, trig_cuePos) && ismember(trig3, trig_loc)
